...
document.addEventListener("DOMContentLoaded", function () {
                    ...
                    document
                        ...
                        .addEventListener("click", function () {
                            ...
                            document
                                ...
                                .classList.remove("is-active"); // Ensuring hamburger is deactivated
                        });
                }
            }
        });
    ...
    // Menu collapse click start
    var topbar_link_list = document.querySelectorAll(
    ...
    if (topbar_link_list) {
        topbar_link_list.forEach((link) => {
            link.addEventListener("click", function (e) {
                var targetElement = e.target;
                setTimeout(function () {
                    targetElement.parentNode.children[1].removeAttribute(
                        "style"
                    );
                }, 1000);
            });
...
(function ($) {
    $.fn.dynamicForm = function (options) {
        const settings = $.extend(
            {
                fetchSources: [], // contoh: [{name:"Kategori Produk", url:"/api/kategori"}]
            },
            options
        );
        ...
        function addRow(initialData = null) {
            const row = $(`
        <div class="row g-2 align-items-center mb-3 dynamic-row">
            <div class="col-md-3">
            ...
            <div class="col-md-5 inputArea">
                <input type="text" class="form-control inputField" name="field_value[]" placeholder="Masukkan teks">
            </div>

            <div class="col-md-2 sourceArea d-none">
                <select class="form-select sourceSelect">
                    <option selected disabled>Pilih sumber data</option>
                </select>
            </div>

            <div class="col-md-2">
                <button class="btn btn-danger removeRow" type="button">Hapus</button>
            </div>
        </div>
    `);
            ...
            // isi dropdown sumber data
            settings.fetchSources.forEach((src) => {
                row.find(".sourceSelect").append(
                    `<option value="${src.url}">${src.name}</option>`
                );
            });

            // EVENT handler bawaan
            row.find(".typeSelect").on("change", function () {
                const type = $(this).val();
                const inputArea = row.find(".inputArea");
                const sourceArea = row.find(".sourceArea");

                if (type === "text") {
                    sourceArea.addClass("d-none");
                    inputArea.html(
                        `<input type="text" class="form-control inputField" name="field_value[]" placeholder="Masukkan teks">`
                    );
                } else {
                    sourceArea.removeClass("d-none");
                    inputArea.html(`
                <select class="form-select inputField" multiple>
                    <option disabled>Silakan pilih sumber data...</option>
                </select>
            `);
                }
            });

            row.find(".sourceSelect").on("change", function () {
                const url = $(this).val();
                const sourceName = $(this)
                    ...
                    .replace(/\s+/g, "_")
                    .toLowerCase();
                const selectTarget = row.find(".inputField");

                selectTarget.attr("name", `field_value[${sourceName}][]`);
                selectTarget.html(`<option>Loading...</option>`);
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $.each(data, function (i, item) {
                            const selected = initialData?.values?.includes(
                                item.value.toString()
                            )
                                ? "selected"
                                : "";
                    ...
                    error: function () {
                        selectTarget.html(
                            `<option>Error mengambil data</option>`
                        );
                    },
                });
            });
            ...
            // AUTO-APPLY INITIAL DATA saat update
            if (initialData) {
                row.find(".typeSelect").val("select").trigger("change");

                const url = settings.fetchSources.find(
                    (src) => src.key === initialData.key
                )?.url;
                if (url) {
                    row.find(".sourceSelect").val(url).trigger("change");
                }
            }
        }
        ...
        this.data("plugin_dynamicForm", { addRow });
        return this;
    };
})(jQuery);

function formatRupiah(angka) {
    const formater = new Intl.NumberFormat("ID", {
        style: "currency",
        currency: "IDR",
